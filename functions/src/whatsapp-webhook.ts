...
async function salvarMensagem(
  ...
  await db.collection('whatsapp_mensagens').add({
    ...
    processada: !erro,
    erro: erro || null,
    recebidasEm: Timestamp.now(),
  });
}

// ─── Processadores de Comando ─────────────────────────────────────────────────

async function processarTarefa(de: string, corpo: string) {
...
async function processarReuniao(de: string, corpo: string) {
  const p = partes(corpo);
  ...
  const ref = await db.collection('reunioes').add({
    titulo,
...
async function processarAula(de: string, corpo: string) {
  const p = partes(corpo);
  ...
  const ref = await db.collection('aulas').add({
    titulo,
    disciplina,
...
async function processarFeedback(de: string, corpo: string) {
  ...
  const ref = await db.collection('feedbacks').add({
    ...
    combinados: [],
    data: Timestamp.now(),
    proximaRevisao: Timestamp.fromDate(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)),
    criadoEm: Timestamp.now(),
  });
  await salvarMensagem(de, corpo, 'feedback', 'feedbacks', ref.id);
}

async function processarViagem(de: string, corpo: string) {
  const p = partes(corpo);
  ...
  const ref = await db.collection('viagens').add({
    destino,
    descricao: `Criada via WhatsApp por ${de}`,
    dataIda: Timestamp.fromDate(dataIda),
    dataVolta: dataVolta ? Timestamp.fromDate(dataVolta) : null,
    orcamento,
    gastoReal: 0,
    status: 'planejada',
    atividades: [],
...
async function processarCusto(de: string, corpo: string) {
  const p = partes(corpo);
  ...
  const ref = await db.collection('custos').add({
    descricao,
    valor,
    categoria,
    tipo,
    ...
    if (req.method === 'POST') {
      try {
        ...
        for (const message of messages) {
          const de: string = message.from || 'desconhecido';
          const corpo: string = message?.text?.body || '';
          ...
          try {
            if (corpoNorm.startsWith('#tarefa')) {
              ...
              await salvarMensagem(de, corpo, 'desconhecido', undefined, undefined,
                'Comando não reconhecido. Use: #tarefa, #reuniao, #aula, #feedback, #viagem ou #custo');
            }
          } catch (err: any) {
