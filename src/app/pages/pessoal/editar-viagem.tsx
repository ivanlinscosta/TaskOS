...
import {
  ...
  CreditCard, Banknote, ChevronDown, ChevronUp, Loader,
} from 'lucide-react';
...
const defaultItens = (): ItemOrcamentoForm[] =>
...
export function EditarViagem() {
  ...
  const [itensOrcamento, setItensOrcamento] = useState<ItemOrcamentoForm[]>(defaultItens());
    ...
    viagensService
      ...
      .then((viagem) => {
        ...
        setForm({
          ...
          dataIda: toDateInput(viagem.dataIda),
          dataVolta: toDateInput(viagem.dataVolta),
        ...
        setItensOrcamento(
          defaultItens().map((item) => {
            const existing = (viagem.orcamentoDetalhado || []).find(
              (i) => i.categoria === item.categoria
            );
            if (!existing) return item;
            return {
              ...item,
              valor: existing.valor > 0 ? existing.valor.toString() : '',
              formaPagamento: existing.formaPagamento,
              parcelas: existing.parcelas?.toString() || '1',
              dataPrimeiraParcela: existing.dataPrimeiraParcela || '',
              aberto: existing.valor > 0,
            };
          })
        );
      })
  ...
  const addAtividade = () => {
    ...
    setAtividades((prev) => [...prev, { id: crypto.randomUUID(), ...novaAtividade }]);
  ...
  const totalOrcamento = itensOrcamento.reduce((s, i) => s + (parseFloat(i.valor) || 0), 0);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-[var(--theme-foreground)]">
            Editar Viagem{form.destino ? ` — ${form.destino}` : ''}
          </h1>
              ...
              <CardContent className="space-y-3">
                {itensOrcamento.map((item) => {
                  ...
                  const ultimaParcela =
                    ...
                    parseInt(item.parcelas) > 1
                      ? calcUltimaParcela(item.dataPrimeiraParcela, parseInt(item.parcelas))
                      : null;
                  const valorParcela =
                    item.formaPagamento === 'a_prazo' && parseInt(item.parcelas) > 0
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                              ...
                              <Select
                                ...
                                onValueChange={(v) =>
                                  updateItem(item.id, 'formaPagamento', v)
                                }
                              >
                                <SelectTrigger className="mt-1">
                                  <SelectValue />
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <Input
                                    ...
                                    value={item.parcelas}
                                    onChange={(e) =>
                                      updateItem(item.id, 'parcelas', e.target.value)
                                    }
                                    className="mt-1"
                                  />
                                </div>
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      updateItem(item.id, 'dataPrimeiraParcela', e.target.value)
                                    }
                                    className="mt-1"
                                  />
                                </div>
                                  ...
                                  {item.dataPrimeiraParcela && (
                                    <div className="flex justify-between text-sm">
                                      ...
                                      <span className="font-medium text-[var(--theme-foreground)]">
                                        {format(
                                          new Date(item.dataPrimeiraParcela),
                                          "d 'de' MMMM 'de' yyyy",
                                          { locale: ptBR }
                                        )}
                                      </span>
                ...
                >
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setNovaAtividade((a) => ({ ...a, nome: e.target.value }))
                      }
                      onKeyDown={(e) =>
                        e.key === 'Enter' && (e.preventDefault(), addAtividade())
                      }
                      className="mt-1"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setNovaAtividade((a) => ({ ...a, data: e.target.value }))
                        }
                        className="mt-1"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setNovaAtividade((a) => ({ ...a, horario: e.target.value }))
                        }
                        className="mt-1"
                      />
                    </div>
                            ...
                            <p className="text-xs text-[var(--theme-muted-foreground)] mt-0.5 flex items-center gap-2">
                              {a.data && (
                                <span className="flex items-center gap-1">
                                  ...
                                  {format(new Date(a.data), "d 'de' MMM 'de' yyyy", {
                                    locale: ptBR,
                                  })}
                                </span>
                              )}
