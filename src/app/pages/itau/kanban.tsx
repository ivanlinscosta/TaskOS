...
export function KanbanItau() {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [tarefas] = useState(mockTarefas.filter(t => t.contexto === 'itau'));
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[var(--theme-foreground)]">Kanban Itaú</h1>
          <p className="text-[var(--theme-muted-foreground)] mt-1">
            Gerencie sprints e entregas corporativas
          </p>
        </div>
        <Button variant="theme" className="gap-2" onClick={() => navigate('/itau/kanban/nova')}>
